...
const CHUNKS_DIR = path.join(__dirname, "..", "data-runtime", "chunks");
...
// OpenAI's text-embedding-3-small does NOT use search_query/search_document
// prefixes — query and document share the same embedding space.
...
function loadAllChunks(): ChunkWithEmbedding[] {
  ...
  if (all.length === 0 || !all[0].embedding) {
    throw new Error(
      "No embeddings found. Run `npx tsx artifacts/web/scripts/reembed-with-openai.ts` first.",
    );
  }
  return all;
}
...
async function runSelfRetrieval(chunks: ChunkWithEmbedding[]): Promise<boolean> {
...
async function runTopicalQueries(chunks: ChunkWithEmbedding[]): Promise<{
  ...
  console.log("\n=== Summary ===");
  console.log(
    `Topical pass rate: ${passedQueries}/${TEST_QUERIES.length} queries had ≥2 expected guests in top-5 (target ≥4/5)`,
  );
  console.log(`Top-1 cosine across ${TEST_QUERIES.length} queries:`);
  console.log(`  median: ${median.toFixed(3)} (target ≥0.4)`);
  console.log(`  mean:   ${mean.toFixed(3)}`);
  console.log(`  min:    ${min.toFixed(3)}`);
  console.log(`  max:    ${max.toFixed(3)}`);

  return { passedQueries, median };
}
...
async function main() {
  ...
  console.log(
    `Loaded ${chunks.length} chunks across ${new Set(chunks.map((c) => c.slug)).size} guests (dim=${dim})`,
  );
  ...
  const selfPass = await runSelfRetrieval(chunks);
  const { passedQueries, median } = await runTopicalQueries(chunks);

  const overallPass = selfPass && passedQueries >= 4 && median >= 0.4;
  console.log(
    `\nOverall: ${overallPass ? "🟢 PASS — embeddings ready to depend on" : "🟡 BELOW BAR — investigate before relying on retrieval"}`,
  );
}
